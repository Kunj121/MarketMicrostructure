...
def find_params_folder(lr_g: float, lr_d: float):
    """
    Look up logs/params_dict.csv and return the PARAMS_x folder name
    matching lr_g, lr_d.
    """
    logs_path = Path(__file__).resolve().parent.parent / "logs" / "params_dict.csv"

    if not logs_path.exists():
        raise FileNotFoundError(f"{logs_path} does not exist. Run training first.")

    df = pd.read_csv(logs_path)

    match = df[(df[' lr_g'] == lr_g) & (df[' lr_d'] == lr_d)]
    if match.empty:
        raise ValueError(f"No PARAMS folder found for lr_g={lr_g}, lr_d={lr_d}")

    params_idx = int(match.iloc[0]['params'])
    return f"PARAMS_{params_idx}"
...
def run_gan_test(lr_g, lr_d, stock):
